...
describe('createQrPayload', () => {
  ...
  it('rejette un sous-tag additionnel invalide', () => {
    expect(() =>
      ...
      'Le sous-tag additional data "0@" doit contenir exactement 2 caractères alphanumériques.'
    );
  });
...
describe('isValidPispiQrPayload diagnostics', () => {
  ...
  it('signale une erreur lors du décodage des informations marchand', () => {
    ...
    expect(
      result.errors.some((error) =>
        error.includes("Erreur lors de l'analyse des informations marchand")
      )
    ).toBe(true);
  });
  ...
  it('accepte une payload sans referenceLabel (Tag 05 optionnel)', () => {
    ...
    const additionalEntries = decodeAdditional(additional!.value).filter(
      (entry) => entry.tag !== '05'
    );
  ...
  it('signale un merchantChannel manquant dans les données additionnelles', () => {
    ...
    expect(result.errors).toContain(
      'Tag 11 (Merchant Channel) manquant dans les données additionnelles.'
    );
  });
  ...
  it('signale une erreur lors du décodage des données additionnelles', () => {
    ...
    expect(
      result.errors.some((error) =>
        error.includes("Erreur lors de l'analyse des données additionnelles")
      )
    ).toBe(true);
  });
  ...
  it('capture une exception inattendue lors du décodage', () => {
    ...
    expect(result.valid).toBe(false);
    expect(
      result.errors.some((error) => error.includes("Erreur lors de l'analyse de la payload"))
    ).toBe(true);
